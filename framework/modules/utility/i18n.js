...
M.I18N = M.Object.extend(/** @scope M.I18N.prototype */ {
    ...
    setCurrentLanguageCode: function( languageCode ) {
        ...
        if( languageCode && languageCode == this._currentLanguageCode ) {
            M.Logger.log('Language \'' + languageCode + '\' already selected');
            return;
        }

        if( !this.isLanguageAvailable(languageCode) ) {
            M.Logger.warn('There is no language \'' + languageCode + '\' specified using default language \'' + this._defaultLanguageCode + '\' instead!');
    ...
    isLanguageAvailable: function( languageCode ) {
        ...
        if( lang && _.isObject(lang) ) {
            ...
            M.Logger.warn('No language \'' + languageCode + '\' specified.');
            return NO;
        }
    },
    ...
    _localize: function( key, context ) {
        ...
        if( !translation ) {
            ...
            if( defaultLanguage && defaultLanguage[key] ) {
                M.Logger.log('Key \'' + key + '\' not defined for language \'' + this._currentLanguageCode + '\', switched to default language \'' + this._defaultLanguageCode + '\'');
                translation = defaultLanguage[key];
            }
        }

        if( !translation ) {
            M.Logger.log('Key \'' + key + '\' not defined for both language \'' + this._currentLanguageCode + '\' and the system\'s default language \'' + this._defaultLanguageCode + '\'');
            translation = key;
        }
        ...
        if( context ) {
            try {
                ...
                M.Logger.error('Error in I18N: Check your context object and the translation string with key "' + key + '". Error Message: ' + e);
            }
        }
